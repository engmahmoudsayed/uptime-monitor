...
handlers._tokens.post = function(data, callback){
  // Check that phone and password fields are valid
  const phone = typeof(data.payload.phone) == 'string' && data.payload.phone.trim().length == 10 ? data.payload.phone.trim() : false;
  const password = typeof(data.payload.password) == 'string' && data.payload.password.trim().length > 0 ? data.payload.password.trim() : false;
  if(phone && password){
    // Lookup the user who matches provided phone number
    _data.read('users', phone, function(err, userData){
      if (!err && userData){
        // Hash the provided password and compare to password hash stored in the user object.
        // Hash the password
        const hashedPassword = helpers.hash(password);
...
handlers._tokens.get = function(data, callback){
  ...
  if(id){
    ...
    callback(400, {'Error' : 'Missing id field (required)'});
  }
};
...
// Container for all checks methods (private)
handlers._checks = {};
  ...
  if(protocol && url && method && successCodes && timeoutSeconds){
    ...
    _data.read('tokens', token, function(err, tokenData){
      if(!err && tokenData){
        ...
        _data.read('users', userPhone, function(err, userData){
          if(!err && userData){
            ...
            if(userChecks.length < config.maxChecks){
              ...
              // Create the check object, and include the user's phone (foreign key)
              var checkObject = {
                'id' : checkId,
                'userPhone' : userPhone,
                'protocol' : protocol,
        ...
        callback(403); // 403: NOT AUTHORIZED
      }
    })
  } else {
  ...
  if(id){
    ...
    _data.read('checks', id, function(err, checkData){
      if(!err && checkData){
        // Get the token from the headers
        const token = typeof(data.headers.token) == 'string' ? data.headers.token : false;
        // Verify tha the given token is valid and belongs to the user who created the check
...
handlers._checks.put = function(data, callback){
  // Check for the required id field
  const id = typeof(data.payload.id) == 'string' && data.payload.id.trim().length == 20 ? data.payload.id.trim() : false;
            ...
            _data.delete('checks', id, function(err){
              if(!err){
                ...
                _data.read('users', checkData.userPhone, function(err, userData) {
                  if (!err && userData) {
                    // get the list of checks by id from the user object
                    const userChecks = typeof(userData.checks) == 'object' && userData.checks instanceof Array ? userData.checks : [];
